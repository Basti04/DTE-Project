...
# # # myModel = UNET()
myDataset = DTE(pathToolsTraining, pathPatientsTraining, filenameToTools, filenameToPatients, 1000, False, 384)

for i in range(1000):
    x,_= myDataset.__getitem__(1)
    print(i)

#writeNumpy(x.detach().numpy(), 'C:/DTEProjektpraktikumSebastianHeid/2D + T (mov)/TestImages/x.raw')
#writeNumpy(_.detach().numpy(), 'C:/DTEProjektpraktikumSebastianHeid/2D + T (mov)/TestImages/_.raw')
