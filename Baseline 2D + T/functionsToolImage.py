...
def loadRandomToolScene(pathStent, pathGuidwire):
    ...
    # choose one time step randomly
    stent_rand = stentSequenz[randint(0, stentSequenz.shape[0] - 1)]
    guidwire_rand = guidwireSequenz[randint(0, stentSequenz.shape[0] - 1)]
     

    return stent_rand, guidwire_rand
...
def convolutionGaussian(tool, size_kernel):
    ...
    tool = tool.view(1, tool.shape[0], tool.shape[1])
    print(tool.shape)
    conv = torch.zeros((tool.shape[0],1006,1006))
...
def chooseRandomPatchTool(tool, patchSize):
    ...
    while(x>tool.shape[1] - patchSize):
        x = randint( x_cm - patchSize, x_cm)
    while(y>tool.shape[2] - patchSize):
        y = randint(y_cm - patchSize, y_cm)
